...
FEATURE_CATEGORIES = {
    ...
    "Frequency":  ["psd_peak_freq", "psd_peak_power", "bp_low", "bp_mid", "bp_high"],
...
def plot_step_distributions(X, y_event, y_source, output_dir):
    ...
    for i, src in enumerate(source_list):
        ...
        data_per_event = [sub[sub["event"] == ev]["step_mean"].dropna().values
                          for ev in event_list]
        positions = x_pos + (i - n_src / 2 + 0.5) * width
        ax.boxplot(data_per_event, positions=positions, widths=width * 0.8,
                   patch_artist=True,
...
def plot_pca_scatter_2d(X_scaled, y_event, y_source, output_dir):
    ...
    legend1 = ax.legend(handles=event_handles, title="Event", loc="upper left", fontsize=8)
...
def plot_feature_importance(final_rf, feature_names, output_dir):
    ...
    cat_handles = [mpatches.Patch(color=CAT_COLOURS[c], label=c) for c in CAT_COLOURS]
