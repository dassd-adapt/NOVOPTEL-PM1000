...
def extract_window_features(win: pd.DataFrame, s_ref: np.ndarray) -> dict:
    ...
    if len(non_nan_steps) > 0:
        ...
        burst_count = float(np.sum(non_nan_steps > 3.0 * median_step))
    else:
        burst_count = 0.0
...
>>>>>>> cfb7bf9 (your description of the changes)
    for src in df["source"].unique():
        ...
        ne_data = df.loc[ne_mask, cols_to_normalise].astype(float)
        ...
        for feat in cols_to_normalise:
            ...
            df_out.loc[src_mask, feat] = (
                (df_out.loc[src_mask, feat].astype(float) - ne_mean[feat]) / (ne_std[feat] + 1e-8)
            )
